...
| Plans: family / team workspaces whose members get the plan's access, server-written entitlements, a 30-day trial, gates (task members, seats, daily voice) that switch on only when billing is enforced; store purchase wiring waits for the developer accounts | `docs/PLANS.md`, `lib/ui/plan_screen.dart`, `push-worker/src/plans.mjs`, `firestore.rules` | built; purchases pending store accounts |
